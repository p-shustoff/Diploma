classdef Night_Driver < EV
    properties
        home_Node
        trajectory
    end
    methods
        function obj = Night_Driver(mean_V, Graph, all_nodes)
        ...
        function obj = set_state(obj,time)
              if(((time < obj.departures(2)) || (time >= obj.departures(1))) && obj.state ~= "charging_st")
                 obj.state = "driving";
              elseif ((time >= obj.departures(2) &&  obj.from_Node ~= obj.home_Node) && obj.state ~= "charging_st")
